...
try {
    if (isset($_FILES['form_favi']) && $_FILES['form_favi']['error'] === UPLOAD_ERR_OK) {
        // Obtiene el archivo y su contenido
        $Favicon = file_get_contents($_FILES['form_favi']['tmp_name']);
     
     
        $query = "UPDATE catalogo_configuracion SET favicoin=?";
        
        // Prepara la sentencia SQL
        $stmt = $mysqli->prepare($query);

        // Vincula los parámetros a los marcadores de posición
        $stmt->bind_param("s", $Favicon); // "b" indica un dato binario
