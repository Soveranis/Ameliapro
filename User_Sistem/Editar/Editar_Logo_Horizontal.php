...
try {
    if (isset($_FILES['logo_horizontal']) && $_FILES['logo_horizontal']['error'] === UPLOAD_ERR_OK) {
        // Obtiene el archivo y su contenido
        $Logo = file_get_contents($_FILES['logo_horizontal']['tmp_name']);
     
        $query = "UPDATE catalogo_configuracion SET Logo_Secundario=?";
        
        // Prepara la sentencia SQL
        $stmt = $mysqli->prepare($query);

        // Vincula los parámetros a los marcadores de posición
        $stmt->bind_param("s", $Logo); // "b" indica un dato binario
