...
try {
    if (isset($_POST['Fecha_limite_pago'])) {
        $Fecha_limite_pago = $_POST['Fecha_limite_pago'];
        $Nombre_instituto = $_POST['Nombre_instituto'];

        $query = "UPDATE catalogo_configuracion SET Nombre_Dependencia=?,Fecha_Limite_Pago=?";
        
        // Prepara la sentencia SQL
        $stmt = $mysqli->prepare($query);

        // Vincula los parámetros a los marcadores de posición
        $stmt->bind_param("ss", $Nombre_instituto,$Fecha_limite_pago);
